...
const products = [
  {
    id: "hoodie1",
    name: "Huanger Films Hoodie",
    price: 45.00,
    image: "images/hoodie.jpg",
    stripe: "https://buy.stripe.com/test_12345" // optional
  },
  {
    id: "shirt1",
    name: "Huanger T-Shirt",
    price: 25.00,
  ...
  products.forEach(product => {
    ...
    card.innerHTML = `
      ...
      <button class="pay-btn paypal-btn"
        onclick="payWithPayPal('${product.id}', ${product.price})">
        Pay with PayPal
      </button>

      ${
        product.stripe
          ? `<a href="${product.stripe}" target="_blank">
               <button class="pay-btn stripe-btn">Pay with Stripe</button>
             </a>`
          : ""
      }
    `;
...
/* ---------------------------
   ...
   (Option A — No SDK needed)
   --------------------------- */

// YOUR PayPal business email:
const PAYPAL_EMAIL = "lucas042213@gmail.com";

function payWithPayPal(itemID, price) {
  const url =
    `https://www.paypal.com/cgi-bin/webscr?` +
    `cmd=_xclick&business=${encodeURIComponent(PAYPAL_EMAIL)}` +
    `&item_name=${encodeURIComponent(itemID)}` +
